from flask import Flask, request, jsonify, send_file
from flask_cors import CORS
import chatbot_db  # chatbot_db 모듈에서 데이터베이스와 GPT 함수 사용
import os
from dotenv import load_dotenv

# .env 파일 로드 (API 키와 환경 변수 설정)
load_dotenv()

# Flask 애플리케이션 초기화
app = Flask(__name__)
CORS(app)  # CORS 활성화 (프론트엔드와 통신 가능)

# 데이터베이스 초기화
print("Initializing database...")
chatbot_db.initialize_database()
print("Database initialized successfully.")

@app.route("/chat", methods=["POST"])
def chat():
    """
    사용자 입력을 받아 GPT 응답을 생성하는 API 엔드포인트
    """
    try:
        # JSON 데이터 가져오기
        data = request.json
        if not data:
            return jsonify({"error": "No data provided"}), 400

        # 사용자 ID와 입력값 가져오기
        user_id = data.get("user_id", "default_user")  # 기본값 설정
        user_input = data.get("user_input")

        if not user_input:
            return jsonify({"error": "User input is required"}), 400

        # GPT 응답 생성 (chatbot_db 모듈 사용)
        response = chatbot_db.get_gpt_response(user_id, user_input)

        # 응답 반환
        return jsonify({"response": response})

    except Exception as e:
        # 오류 발생 시 JSON으로 반환
        return jsonify({"error": str(e)}), 500


@app.route("/history", methods=["GET"])
def chat_history():
    """
    특정 사용자의 전체 대화 기록을 가져오는 API
    """
    try:
        user_id = request.args.get("user_id", "default_user")  # URL 파라미터로 user_id 받음
        history = chatbot_db.get_recent_memory(user_id, limit=50)  # 최근 50개 대화 조회

        return jsonify({"user_id": user_id, "history": history})

    except Exception as e:
        return jsonify({"error": str(e)}), 500


@app.route("/download_db", methods=["GET"])
def download_db():
    """
    Render 서버에서 SQLite 데이터베이스 파일을 다운로드할 수 있도록 제공
    """
    try:
        db_path = "chat_memory.db"  # 데이터베이스 파일 경로
        if not os.path.exists(db_path):
            return jsonify({"error": "Database file not found"}), 404

        return send_file(db_path, as_attachment=True)

    except Exception as e:
        return jsonify({"error": str(e)}), 500


if __name__ == "__main__":
    # Flask 서버 실행
    app.run(host="0.0.0.0", port=5000, debug=True)

